...
// Tính số lượng sản phẩm trong giỏ để hiển thị badge trên icon ở navbar trang login
$loginCart = getCart();
$loginCartCount = 0;
if (!empty($loginCart)) {
...
if(isset($_POST['username']))
{
    ...
    if($result->num_rows > 0)
    {
        ...
        // Lưu lại giỏ hàng hiện tại (khách) rồi gắn vào tài khoản sau khi đăng nhập
        $currentCart = getCart();

        $_SESSION['user'] = $row;

        // Sau khi có user trong session, saveCart sẽ lưu giỏ hàng theo tài khoản
        ...
        $sql = "SELECT * FROM tai_khoan
            WHERE ten_dang_nhap='$username'
            AND mat_khau='$password' AND vai_tro='admin'";
        $result = $conn->query($sql);
        if($result->num_rows > 0){
