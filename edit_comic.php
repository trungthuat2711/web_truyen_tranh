...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $ban_chay = isset($_POST['ban_chay']) ? $_POST['ban_chay'] : 0;

    // giữ ảnh cũ
    $anh = $sp['anh_sp'];

    // nếu có upload ảnh mới
    if(isset($_FILES['anh_sp']) && $_FILES['anh_sp']['error'] == 0 && isset($_FILES['anh_chi_tiet_sp']) && $_FILES['anh_chi_tiet_sp']['error'] == 0){
        $anh = time() . "_" . $_FILES['anh_sp']['name'];
        $target = __DIR__ . "/uploads/" . $anh;
        move_uploaded_file($_FILES['anh_sp']['tmp_name'], $target);
        
        $anh_chi_tiet = 'big_'.$anh;
...
<div class="content">
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="form-row">
            ...
            <div class="form-group">
                <label>Giá *</label>
                <input type="text" name="gia" value="<?= number_format($sp['gia']) ?>" required>
            </div>

            <div class="form-group">
                <label>Giá cũ</label>
                <input type="text" name="gia_cu" value="">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="ma_loai" required>
                    ...
                    <option value="<?= $l['ma_loai'] ?>" <?= $l['ma_loai'] == $sp['ma_loai'] ? 'selected' : '' ?>>
                        <?= $l['ten_loai'] ?>
                    </option>
                    <?php endwhile; ?>
                </select>
            ...
            <div class="form-group flex-row justify-content-between">
                <div class="card">
                    <label class="mb-3">Ảnh hiện tại</label><br>
                    <img src="uploads/<?= $sp['anh_sp'] ?>" width="120">
                </div>
                <div class="card">
                    <label class="mb-3">Ảnh chi tiết hiện tại</label>
                    <img src="uploads/big_<?= $sp['anh_sp'] ?>" width="120">
                </div>
            </div>
            ...
            <div class="form-group">
                <label>Đổi ảnh chi tiết mới</label>
                <input type="file" name="anh_chi_tiet_sp">
                <br><br>
            </div>
