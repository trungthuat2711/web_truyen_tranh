...
if ($singleMode) {
    $stmt = $conn->prepare('SELECT ma_sp, ten_sp, gia, anh_sp, so_luong_ton FROM san_pham WHERE ma_sp = ? AND trang_thai = 1 AND so_luong_ton > 0 LIMIT 1');
    if ($stmt) {
    ...
    if (!$product) {
        flash('Sản phẩm không tồn tại hoặc đã hết hàng.','danger');
        header('Location: product-detail.php?id=' . (int)$singleProductId);
        exit;
    }

    if ($singleQuantity > (int)$product['so_luong_ton']) {
        flash('Số lượng mua vượt quá tồn kho hiện tại.','danger');
        header('Location: product-detail.php?id=' . (int)$singleProductId);
        exit;
    }
    ...
    if (empty($cart) || empty($productsById)) {
        flash('Giỏ hàng trống. Vui lòng thêm sản phẩm trước khi thanh toán.','danger');
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            foreach ($cart as $item) {
                ...
                //here
                $stmUpdate = $conn->prepare("UPDATE san_pham SET so_luong_ton = GREATEST(so_luong_ton - ?,0) WHERE ma_sp = ?");
                $stmUpdate->bind_param("ii",$quantity,$productId);
                $stmUpdate->execute();
                $stmUpdate->close();

                $stmStatus = $conn->prepare("UPDATE san_pham SET trang_thai = CASE WHEN so_luong_ton <= 0 THEN 0 ELSE 1 END WHERE ma_sp = ?");
                $stmStatus->bind_param("i", $productId);
                $stmStatus->execute();
                $stmStatus->close();
            }
            $stmtItem->close();
...
<div class="container py-5">
    ...
    <div class="alert alert-<?php echo $flashMessage[1];?>" role="alert">
                ...
                <form method="post" novalidate>
                    ...
                    <div class="card p-4 mb-4">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="fullname"
                                    class="form-control form-ctrl-qty <?php echo isset($errors['fullname']) ? 'is-invalid' : ''; ?>"
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <input type="email" name="email"
                                    class="form-control form-ctrl-qty <?php echo isset($errors['email']) ? 'is-invalid' : ''; ?>"
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="tel" name="phone"
                                    class="form-control form-ctrl-qty <?php echo isset($errors['phone']) ? 'is-invalid' : ''; ?>"
                                    value="<?php echo htmlspecialchars($values['phone']); ?>" pattern="[0-9]{10,}"
                                    required>
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="address"
                                    class="form-control form-ctrl-qty <?php echo isset($errors['address']) ? 'is-invalid' : ''; ?>"
                                    value="<?php echo htmlspecialchars($values['address']); ?>" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="notes" class="form-control form-ctrl-qty"
                                rows="3"><?php echo htmlspecialchars($values['notes']); ?></textarea>
                        </div>

                        <div class="form-group">
                            <label class="form-label">Phương thức thanh toán</label>

                            <div class="payment-method <?php echo $values['payment'] === 'cod' ? 'active' : ''; ?>"
                                data-value="cod" onclick="selectPayment('cod')">
                                <input id="paymentCod" type="radio" name="payment" value="cod"
                                    <?php echo $values['payment'] === 'cod' ? 'checked' : ''; ?>>
                            ...
                            <div class="payment-method <?php echo $values['payment'] === 'bank' ? 'active' : ''; ?>"
                                data-value="bank" onclick="selectPayment('bank')">
                                <input id="paymentBank" type="radio" name="payment" value="bank"
                                    <?php echo $values['payment'] === 'bank' ? 'checked' : ''; ?>>
